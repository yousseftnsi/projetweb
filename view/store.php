...
<head>
  ...
  <body>
        ...
        <input type="hidden" name="id">
          ...
          <!-- Product Price -->
            ...
            <input type="text" id="description" name="description"  >
             ...
             <input data-name="#gallery2" id="image" class="form_gallery-upload"
    ...
    const idCat = document.querySelector("select[name='id_cat']");
    ...
    if (name.value.trim().length < 3) {
        setError(name, "Le titre du produit doit comporter au moins 3 caractères.");
    }

    // Validate Price
    if (!price.value || parseFloat(price.value) <= 0) {
        setError(price, "Le prix doit être un nombre positif.");
    }

    // Validate Category Selection
    if (!idCat.value) {
        setError(idCat, "Vous devez sélectionner une catégorie.");
    }

    // Validate Description
    if (description.value.trim().length < 5) {
        setError(description, "La description doit comporter au moins 5 caractères.");
    }

    // Validate Image
    if (!image.files[0]) {
        setError(image, "Vous devez télécharger une image.");
    } else {
        const validExtensions = ["image/png", "image/jpeg", "image/jpg"];
        if (!validExtensions.includes(image.files[0].type)) {
            setError(image, "Le fichier d'image doit être au format .png, .jpg ou .jpeg.");
        }
    }
